                 ...
                 $tab_staf="<table  id=tab1 CELLPADDING=10 CELLSPACING=0 align=center width=90%>
                  ...
                  foreach($count_staf as $k=>$v)
                   ...
                   if($_COOKIE['mov']=="1")
                    {
            	     $i+=$conf[7];
            	     fwrite($f,"Общая стоимость заказа с доставкой $i&nbsp;$conf[8]\r\n");
                    }
                   else
                    {
                     fwrite($f,"Общая стоимость заказа без доставки $i&nbsp;$conf[8]\r\n");
                    }

                  if($conf[1]!="")fwrite($f,"Фамилия*$_POST[fam]\r\n");
                  if($conf[2]!="")fwrite($f,"Имя*$_POST[nam]\r\n");
                  if($conf[3]!="")fwrite($f,"E-mail*$_POST[user_mail]\r\n");
                  if($conf[4]!="")fwrite($f,"Телефон*$_POST[fone]\r\n");
                  if($conf[5]!="")fwrite($f,"Адрес*$_POST[adr]\r\n");
                  if($conf[6]!="")fwrite($f,"Ближайшее метро*$_POST[metr]");
           ...
           foreach($count_staf as $k=>$v)
             {               $oll=$price[$k]*$v;
               ...
               <td id=sett1 align=center>$oll&nbsp;$conf[8]</td><td id=sett1 align=center><a href=form.php?d=$k><img src=../bas/admin/img/min.png alt=удалить border=0></a>
               &nbsp;&nbsp;&nbsp;<a href=form.php?add=$k><img src=../bas/admin/img/add.png alt=добавить border=0></a></td></tr>";
               $i+=$oll;
             }
